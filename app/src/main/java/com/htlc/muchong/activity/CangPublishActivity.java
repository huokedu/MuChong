...
public class CangPublishActivity extends BaseActivity implements AdapterView.OnItemClickListener, View.OnClickListener {
    ...
    protected int getLayoutId() {
        return R.layout.activity_cang_publish;
    }

    @Override
    protected void setupView() {
        mTitleTextView.setText(R.string.title_cang_publish);
        ...
        public void onPickPhotoFinishListener(File imageFile) {
            if (imageFile != null) {
                if (isPickCover) {
                    ...
                    Picasso.with(CangPublishActivity.this).load(Uri.fromFile(coverImageFile)).resizeDimen(R.dimen.publish_grid_view_size, R.dimen.publish_grid_view_size).centerCrop().into(imageViewCover);
                } else {
